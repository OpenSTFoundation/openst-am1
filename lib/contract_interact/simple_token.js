...
SimpleTokenKlass.prototype.transfer = async function(
  ...
  const asyncTransfer = async function() {
    ...
    const openStNotification = notificationProvider.getInstance();
    ...
    return new Promise(function(onResolve, onReject) {
      const onReceipt = function(receipt) {
        ...
        openStNotification.publishEvent.perform(notificationData).catch(function(err) {
          logger.error('Message for transaction_mined was not published in simple_token. Error: ', err);
        });
      ...
      web3ProviderFactory
        ...
        .then(function() {
          return web3ProviderFactory
            ...
            .on('transactionHash', function(transactionHash) {
              ...
              openStNotification.publishEvent.perform(notificationData).catch(function(err) {
                logger.error('Message for transaction initiated was not published in simple_token. Error: ', err);
              });
        ...
        .catch(async function(reason) {
          ...
          const onCatchError = function() {
            ...
            openStNotification.publishEvent.perform(notificationData).catch(function(err) {
              logger.error('Message for transaction error was not published in simple_token. Error: ', err);
            });

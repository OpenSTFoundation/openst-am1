...
BrandedTokenKlass.prototype = {
  ...
  getBalanceOf: async function(owner) {
    ...
    const balanceResponse = await new openSTStorage.cache.TokenBalance({
      ...
      shard_name: oThis.ic().configStrategy.TOKEN_BALANCE_SHARD_NAME
    }).fetch();
      ...
      return new Promise(function(onResolve, onReject) {
        const onReceipt = async function(receipt) {
          ...
          openStNotification.publishEvent.perform(notificationData).catch(function(err) {
            logger.error('Message for transaction mined was not published in branded_token. Error: ', err);
          });
        ...
        web3Provider.eth.personal
          ...
          .then(function() {
            return web3Provider.eth
              ...
              .on('transactionHash', function(transactionHash) {
                ...
                openStNotification.publishEvent.perform(notificationData).catch(function(err) {
                  logger.error('Message for transaction initiated was not published in branded_token. Error: ', err);
                });
          ...
          .catch(async function(reason) {
            ...
            const onCatchError = async function() {
              ...
              openStNotification.publishEvent.perform(notificationData).catch(function(err) {
                logger.error('Message for transaction error was not published in branded_token. Error: ', err);
              });
  ...
  claim: async function(senderAddress, senderPassphrase, beneficiaryAddress) {
    ...
    claimResponse.data.formattedEvents = formattedEvents;
